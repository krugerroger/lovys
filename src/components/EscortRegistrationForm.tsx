...
export default function EscortRegistrationForm({
  ...
  const t = useScopedI18n('Auth.EscortRegistration' as never) as (key: string, vars?: Record<string, any>) => string;
    ...
    try {
      ...
      if (result.success) {
        if (result.requiresEmailVerification) {
          ...
          toast.success(
            t('messages.emailVerificationSent'),
            {
              description: result.message || t('messages.emailVerificationDescription')
            }
          );
        } else {
      ...
      toast.error(
        ...
        {
          description: error instanceof Error ? error.message : t('messages.genericError')
        }
      );
      
      setErrors({ 
        submit: error instanceof Error ? error.message : t('messages.genericError')
      });
    } finally {
            ...
            <form onSubmit={handleSubmit} className="space-y-7">
              ...
              <div>
                <label className="block text-white text-sm font-medium mb-3">
                  {t('fields.username')} <span className="text-red-500">{t('fields.required')}</span>
                </label>
              ...
              <div>
                <label className="block text-white text-sm font-medium mb-3">
                  {t('fields.email')} <span className="text-red-500">{t('fields.required')}</span>
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <div>
                <label className="block text-white text-sm font-medium mb-3">
                  {t('fields.password')} <span className="text-red-500">{t('fields.required')}</span>
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <div>
                <label className="block text-white text-sm font-medium mb-3">
                  {t('fields.confirmPassword')} <span className="text-red-500">{t('fields.required')}</span>
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <Lock className="w-5 h-5 text-gray-500 group-focus-within:text-pink-400 transition-colors" />
